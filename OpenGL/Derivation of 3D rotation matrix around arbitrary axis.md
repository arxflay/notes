Rotation about an arbitrary axis involves getting a 2D plane from vector $j$ and axis $\hat n$, around which rotation will be performed. Remember, **a plane is just a slice of 3D space**.

To determine vector $j'$, we need $j'_{perpendicular}$ and $j'_{parallel}$. 
1. If $j_{parallel}$ is parallel to vector $\hat n$, then rotation will not change that component (since it's rotation around $\hat n$). That means that $j_{parallel}$ is equal to $j'_{parallel}$. $j'_{parallel}$ can be computed using the dot product $(\hat n*j)\hat n$. 
2. We need to compute $j_{perpendicular}$ which can be computed as $j - j_{parallel}$. 
3. Using vector $j_{perpendicular}$, we compute vector $w$, which is vector $j_{perpendicular}$ rotated by $90\textdegree$ around $\hat n$ (perpendicular to both $\hat n$ and $j_{perpendicular}$). We can use the cross product to compute $w$. 
...
4. $w = \hat n \times j_{perpendicular} = \hat n \times (j-j_{parallel}) = \hat n  \times j$ (since the cross product of a parallel vector is equal to 0)
...
11. Compute the p,q,r (x,y,z) components
**basis p**
